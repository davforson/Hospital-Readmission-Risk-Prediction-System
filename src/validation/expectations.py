...
def validate_patients(df: pd.DataFrame) -> dict: 
    ...
    suite = context.suites.add(
        gx.ExpectationSuite(name = "patient_validation_suite")
    )
    ...
    # Date_of_birth: never null, in the past (no future dates)
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(column="date_of_birth")
    )
    ...
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(
            column="zip_code", mostly = 0.95 )
    )

    # Demographic details: race - allow up to 5% null
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(
            column="race", mostly = 0.95
        )
    )
...
def validate_admissions(df: pd.DataFrame) -> dict:
    ...
    suite = context.suites.add(
        gx.ExpectationSuite(name="admissions_validation_suite")
    )

    # admission_id    → unique, not null
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToBeUnique(column="admission_id")
    )

    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(column="admission_id")
    )
    ...
    # discharge_date  → not null
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(column='discharge_date')
    )
    ...
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToBeInSet(
            column="discharge_disposition", value_set = [
                "Home", "SNF", "Rehab", "Home Health", "AMA", "Expired"
            ]
        )
    )
    ...
    # length_of_stay  → not null, minimum value 1 (nobody stays 0 days)
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(column='length_of_stay')
    )
    ...
    # readmitted_30d  → not null (this is our target variable — must be clean)
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToNotBeNull(column='readmitted_30d')
    )

    validation_def = context.validation_definitions.add(
        gx.ValidationDefinition(
            name="validate_admissions",
            data = batch_def,
            suite = suite
        )
    )
...
def validate_lab_results(df: pd.DataFrame) -> dict:
    ...
    suite = context.suites.add(
        gx.ExpectationSuite(
            name="Lab_results_validation_suite"
        )
    )
