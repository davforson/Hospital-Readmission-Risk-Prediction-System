...
def prepare_data(feature_path: str = "data/processed/features.parquet"):
    ...
    if "primary_diagnosis_code" in df.columns:
        df = pd.get_dummies(data = df, columns=['primary_diagnosis_code'],
                             prefix= ['diag'], drop_first=True)
        feature_cols = [c for c in df.columns if c != target_col]
    ...
    logger.info(f"Readmission rate - Train: {y_train.mean()}, Val: {y_val.mean()}, Test: {y_test.mean()}")
